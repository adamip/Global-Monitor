...
WHILE @iDateSlot <= @iToDate
	...
	IF EXISTS( SELECT * FROM sys.objects WHERE object_id = OBJECT_ID( @TableName ) AND TYPE in ( N'U' ))	/* if source table does not exist, then skip */
		...
		SET @SqlCommand = @SqlInsert + @SqlSelect + @SqlFrom + @SqlWhere1 + @SqlWhere2 + @SqlGroup + @SqlOrder;
		...
		WHILE @@FETCH_STATUS = 0
			...
			IF NOT EXISTS ( SELECT * FROM [GMPORTAL\SQLEXPRESS].[GMReports].[dbo].[NTEvent]
				...
				BEGIN
				PRINT 'Inserting record: Agent GUID = ' + CONVERT( NVARCHAR(50), @curAgentGUID ) + ', Event Time = ' + CONVERT( VARCHAR(50), @curEventTime ) + ', Message = ' + @curMessage; 	 
				INSERT INTO [GMPORTAL\SQLEXPRESS].[GMReports].[dbo].[NTEvent]( [AgentGUID], [EventTime], [Message], [RecCreatedOn] ) 
					VALUES ( @curAgentGUID, @curEventTime, @curMessage, GETDATE());
				END
