...
WHILE @@FETCH_STATUS = 0
	...
	IF @Reporting_Enabled LIKE 'YES'
		...
		IF NOT EXISTS ( SELECT * FROM [GMPORTAL\SQLEXPRESS].[GMReports].[dbo].[Servers] 
			WHERE [AgentGUID] = @id
				AND [Server] LIKE @Mach_Name
				AND Company LIKE @Group_Name
				AND TheDate = @Date_Stamp
				AND MachineType LIKE @Machine_Type 
				AND IPAddress LIKE @IP_Address
				AND ( LastReboot IS NULL OR LastReboot = @Last_Reboot )
				AND MemoryInstalledMB = @Memory_Installed_MB
				AND Processor LIKE @Processors
				AND Cores = @Core
				AND WindowsVersion LIKE @Windows_Version )
			BEGIN
