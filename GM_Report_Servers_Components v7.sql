	...
	IF EXISTS( SELECT * FROM sys.objects WHERE object_id = OBJECT_ID( @TableName ) AND TYPE in ( N'U' ))	/* if source table does not exist, then skip */
		...
		WHILE @HourSlot < 24
			...
			IF @CPU = 1
				...
				SET @iTEMP = 0;
				SELECT @iTEMP = COUNT(*) FROM #tmpCPU; 
				PRINT '# of data in #tmpCPU = ' + CONVERT( VARCHAR(10), @iTEMP );
				IF @iTEMP = 0 
					PRINT 'No data on CPU on EventDateTime = ' + CONVERT( VARCHAR, DATEADD( HOUR, @HourSlot, @dDATE )) + ', Hour slot = ' + CONVERT( VARCHAR, @HourSlot);
				ELSE
					BEGIN
					SET @iTEMP = 0;
					PRINT 'DATEADD( HOUR, @HourSlot, @dDATE ) = ' + CONVERT( VARCHAR(30), DATEADD( HOUR, @HourSlot, @dDATE )) + ', DATEADD( HOUR, @HourSlot + 1, @dDATE ) = ' + CONVERT( VARCHAR(30), DATEADD( HOUR, @HourSlot + 1, @dDATE ));	
					SELECT @iTEMP = COUNT(*) FROM [GMPORTAL\SQLEXPRESS].[GMReports].[dbo].[CPU] 
						WHERE [TheDate] >= DATEADD( HOUR, @HourSlot, @dDATE ) AND [TheDate] < DATEADD( HOUR, @HourSlot + 1, @dDATE );
					PRINT '# of data in remote database CPU = ' + CONVERT( VARCHAR(10), @iTEMP );	
					IF @iTEMP = 0 
						BEGIN
						SET @SqlCommand1 = 'INSERT INTO [GMPORTAL\SQLEXPRESS].[GMReports].[dbo].[CPU] ( [AgentGUID], [TheDate], [Hour], [Sum], [DataPoints], [RecCreatedOn] ) ';
						SET @SqlCommand1 = @SqlCommand1 + 'SELECT [AgentGUID], [TheDate], [Hour], [Sum], [DataPoints], GETDATE() FROM [#tmpCPU]';
						PRINT 'Dump records: ' + @SqlCommand1;	
						EXEC( @SqlCommand1 );
						END	
					ELSE
						BEGIN
						...
						DECLARE CurCPU CURSOR FOR	/* SQL Cursor */
							SELECT RecID, [AgentGUID], [TheDate], [Hour], [Sum], [DataPoints] FROM [#tmpCPU] ORDER BY RecID;
						OPEN CurCPU;
						FETCH NEXT FROM CurCPU INTO @curRecID, @curAgentGUID, @curTheDate, @curHour, @curSum, @curDataPoints;
			...
			IF @MEM = 1
				...
				SET @iTEMP = 0;
				SELECT @iTEMP = COUNT(*) FROM #tmpMEM; 
				IF @iTEMP = 0 
					PRINT 'No data on MEM on EventDateTime = ' + CONVERT( VARCHAR, DATEADD( HOUR, @HourSlot, @dDATE )) + ', Hour slot = ' + CONVERT( VARCHAR, @HourSlot);
				ELSE
					BEGIN
					SET @iTEMP = 0;
					SELECT @iTEMP = COUNT(*) FROM [GMPORTAL\SQLEXPRESS].[GMReports].[dbo].[Memory] 
						WHERE [TheDate] >= DATEADD( HOUR, @HourSlot, @dDATE ) AND [TheDate] < DATEADD( HOUR, @HourSlot + 1, @dDATE );
					IF @iTEMP = 0 
						BEGIN
						SET @SqlCommand2 = 'INSERT INTO [GMPORTAL\SQLEXPRESS].[GMReports].[dbo].[Memory] ( [AgentGUID], [TheDate], [Hour], [Sum], [DataPoints], [RecCreatedOn] ) ';
						SET @SqlCommand2 = @SqlCommand2 + 'SELECT [AgentGUID], [TheDate], [Hour], [Sum], [DataPoints], GETDATE() FROM [#tmpMEM]';
						PRINT 'Dump records: ' + @SqlCommand2;	
						EXEC( @SqlCommand2 );
						END	
					ELSE
						BEGIN
						...
						DECLARE CurMEM CURSOR FOR	/* SQL Cursor */
							SELECT RecID, [AgentGUID], [TheDate], [Hour], [Sum], [DataPoints] FROM [#tmpMEM] ORDER BY RecID;
						OPEN CurMEM;
						FETCH NEXT FROM CurMEM INTO @curRecID, @curAgentGUID, @curTheDate, @curHour, @curSum, @curDataPoints;
						WHILE @@FETCH_STATUS = 0
							BEGIN
