...
DECLARE @SqlInsert VARCHAR(150);
DECLARE @SqlSelect VARCHAR(150);
...
DECLARE @SqlCommand1 VARCHAR(3000), @SqlCommand2 VARCHAR(3000);
DECLARE @curRecID BIGINT;	
DECLARE @curAgentGUID Numeric( 26,0 );
DECLARE @curEventTime Datetime;
DECLARE @curMessage NVARCHAR( 2200 );

/* length of Message data field in database */
DECLARE @curMessageLen INT;
SET @curMessageLen = 2048;
...
IF ISDATE( @Parm_ToDate ) = 1
	...
	IF @iDateSlot > @iToDate AND @iToDate > 0
		...
		END
	END
IF @iToDate = 0 SET @iToDate = @iDateSlot;	
...
WHILE @iDateSlot <= @iToDate
	...
	IF EXISTS( SELECT * FROM sys.objects WHERE object_id = OBJECT_ID( @TableName ) AND TYPE in ( N'U' ))	/* if source table does not exist, then skip */
		BEGIN
		SELECT @iTEMP = COUNT(*) FROM [GMPORTAL\SQLEXPRESS].[GMReports].[dbo].[NTEvent] 
			WHERE [EventTime] >= CONVERT( DATETIME, CONVERT( VARCHAR(50), @iDateSlot )) AND
				[EventTime] < DATEADD( DAY, 1, CONVERT( DATETIME, CONVERT( VARCHAR(50), @iDateSlot )));
		IF @iTEMP = 0
			BEGIN
			SET @SqlInsert = 'INSERT INTO [GMPORTAL\SQLEXPRESS].[GMReports].[dbo].[NTEvent]( [AgentGUID], [EventTime], [Message], [RecCreatedOn] ) ';
			SET @SqlSelect = 'SELECT AgentGUID, EventTime, Message, GETDATE() AS RecCreatedOn FROM ' + @TableName + ' ORDER BY EventTime;';
			SET @SqlCommand1 = @SqlInsert + @SqlSelect; 
		...
		ELSE
			...
			SET @SqlSelect = 'SELECT [agentGuid], [eventTime], [message] ';
			...
			WHILE @@FETCH_STATUS = 0
				...
				/* Don't use @curMessage LIKE [Message] because string data might contain wildcard characters such as [ _ % '  */
				SELECT @curAgentGUID = [AgentGUID], @curEventTime = [EventTime], @curMessage = [Message]
					FROM [#tmp] 
					WHERE @curRecID = RecID; 
				/* trying to Insert.  Avoid record duplication */		
				IF EXISTS ( SELECT * FROM [GMPORTAL\SQLEXPRESS].[GMReports].[dbo].[NTEvent]
					WHERE @curAgentGUID = [agentGuid] AND @curEventTime = [EventTime] AND @curMessage = [Message] )
					PRINT 'Record already exists : Guid = ' + CONVERT( NVARCHAR(26), @curAgentGUID ) + ', Time  = ' + CONVERT( VARCHAR(50), @curEventTime ) + ', Message = [' + @curMessage + ']';
				ELSE	/* Actual Insertion.  Avoid record duplication */		
					BEGIN
					SET @curMessage = LEFT( REPLACE( @curMessage, '''', '''''' ), @curMessageLen );			
					SET @SqlCommand2 = 'INSERT INTO [GMPORTAL\SQLEXPRESS].[GMReports].[dbo].[NTEvent]( [AgentGUID], [EventTime], [Message], [RecCreatedOn] ) VALUES ( ' + CONVERT( NVARCHAR(26), @curAgentGUID ) + ', ''' + CONVERT( VARCHAR(50), @curEventTime, 21 ) + ''', ''' + @curMessage + ''', GETDATE());';
